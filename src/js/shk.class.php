    ...
    public function getProductsList( $purchasesData = array(), $chunkName = '' ){
        ...
        if( !empty( $purchasesData ) ){
            ...
            foreach( $purchasesData as $i => $purchase ){
                ...
                $price_city = $this->modx->runSnippet('geo-price', $gp_args);
                
                
                if( $this->config['groupBy'] ){
                    $price_total = $price_count = $price_count_total = $price_city;
                }
                else {
                    $price_total = $price_city + $additPrice;
                    $price_count = $price_city * $purchase['count'];
                    $price_count_total = ( $price_city + $additPrice ) * $purchase['count'];
                    //echo $price_total;
                    //echo $price_count;
                    //echo $price_count_total;
                }
    ...
    public function getTotal(){
        ...
        if( !empty( $this->data ) ){
            ...
            foreach( $this->data as $parchase ){
                ...
                $gp_args = array(
                    'id' => $parchase['id'],
                    'price' => $parchase['price']
                );
                
                $price_city = $this->modx->runSnippet('geo-price', $gp_args);
                
                self::$price_total += $price_city * $parchase['count'];
                self::$items_total += $parchase['count'];
